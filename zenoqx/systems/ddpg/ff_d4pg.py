...
from zenoqx.systems.ddpg.ddpg_types import DDPGOptStates, DDPGModels
...
from zenoqx.utils.loss import categorical_td_learning
...
def get_default_behavior_policy(config: DictConfig) -> Callable:
    def behavior_policy(
        ...
        action = jax.vmap(model)(observation).mode()
    ...
    def _update_step(
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _q_loss_fn(
                ...
                _, q_logits_tm1, q_atoms_tm1 = jax.vmap(q_model)(
                    transitions.obs, transitions.action
                )
                next_action = (
                    jax.vmap(target_actor_models)(transitions.next_obs)
                    .mode()
                    .clip(config.system.action_minimum, config.system.action_maximum)
                )
                _, q_logits_t, q_atoms_t = jax.vmap(target_q_model)(
                    transitions.next_obs, next_action
                )
            ...
            def _actor_loss_fn(
                ...
                o_t = transitions.obs
                a_t = (
                    jax.vmap(actor_models)(o_t)
                    .mode()
                    .clip(config.system.action_minimum, config.system.action_maximum)
                )
                q_value, _, _ = jax.vmap(q_model)(o_t, a_t)
...
def learner_setup(
    ...
    q_model_input = hydra.utils.instantiate(
        config.network.q_network.input_layer,
    )
    ...
    q_network = CompositeNetwork([q_model_input, q_model_torso, q_model_head])
    ...
    q_opt_state = q_optim.init(q_network)

    actor_models = OnlineAndTarget(actor_model, actor_model)
    q_models = OnlineAndTarget(q_network, q_network)
