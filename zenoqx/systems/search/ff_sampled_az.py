...
def make_root_fn(config: DictConfig) -> RootFnApply:
    def root_fn(
...
def make_recurrent_fn(
    ...
    def recurrent_fn(
        ...
        next_sampled_actions = pi.sample(seed=key, sample_shape=config.system.num_samples)
        chex.assert_shape(
...
def make_sampled_search_apply_fn(
    ...
    def search_apply_fn(
        ...
        search_output = mctx_search_apply_fn(models, key, root)
...
def get_learner_fn(
    ...
    def _update_step(learner_state: ZLearnerState, _: Any) -> Tuple[ZLearnerState, Tuple]:
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            actor_grads, actor_loss_info = actor_grad_fn(models.actor_model, sequence, actor_key)
...
def learner_setup(
    ...
    dummy_transition = SampledExItTransition(
        ...
        search_policy=jnp.zeros((config.system.num_samples,)),
