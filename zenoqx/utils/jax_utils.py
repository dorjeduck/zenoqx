...
def unreplicate_batch_dim(x: chex.ArrayTree) -> chex.ArrayTree:
    ...
    In zenoqx's case it is always the second dimension, after the device dimension.
    We simply take element 0 as the params are identical across this dimension.
    """
    return jax.tree.map(lambda x: x[:, 0, ...], x)  # type: ignore


def unreplicate_batch_dim_pytree(tree):

    def _unreplicate_batch_dim(x):
        if isinstance(x, jnp.ndarray):
            return x[0]
        else:
            return x

    return jax.tree_util.tree_map(_unreplicate_batch_dim, tree)
