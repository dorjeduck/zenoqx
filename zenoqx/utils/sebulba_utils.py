...
from zenoqx.base_types import Parameters, ZenoqxTransition
...
class OnPolicyPipeline(threading.Thread):
    ...
    def put(
        self,
        traj: Sequence[ZenoqxTransition],
        timestep: TimeStep,
    ...
    ) -> Tuple[ZenoqxTransition, TimeStep, Dict[str, List[float]], Dict[str, List[float]]]:
        """Get a trajectory from the pipeline."""
        return self._queue.get(block, timeout)  # type: ignore

    @partial(jax.jit, static_argnums=(0,))
    def stack_trajectory(self, trajectory: List[ZenoqxTransition]) -> ZenoqxTransition:
